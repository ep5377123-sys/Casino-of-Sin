...
import Casino_of_Sin_Final
import threading
import winsound

#helps run the game sounds
def _play(beeps):
...
def createRooms():
    ...
    main.game      = None
    roulette.game  = Casino_of_Sin_Final.playRoulette
    blackjack.game = Casino_of_Sin_Final.playBlackjack
    bar.game       = None
    cockfight.game = Casino_of_Sin_Final.startCockfight
    wheel.game     = Casino_of_Sin_Final.spinTheWheel

    bar.shop = Casino_of_Sin_Final.goShop
...
class App(Frame):
    def __init__(self, master):
        ...
        # make the outer grid: main content row (weight 4) + nav bar row (weight 1) (Designed and inspired by Claude)
    ...
    def _build_main_panel(self): #makes the main panel on the left side of the window where the room image and description are displayed (Claude helped me design this layout and build this function)
        ...
        self.money_label = Label(
            self.info_panel,
            text=f"Money:  ${Casino_of_Sin_Final.money}",
        ...
        self.inventory_label = Label(
            self.info_panel,
            text=f"Inventory:  {Casino_of_Sin_Final.inventory}",
        ...
        # Row 8 — Action buttons container (all action buttons live here)
    ...
    # helper functions for building UI components built by Claude

    def _separator(self, parent, row): # helper to add a horizontal separator line
        Frame(parent, bg=ACCENT, height=1).grid(
            row=row, column=0, sticky="ew", padx=14
        )

    def _btn(self, parent, text, bg, fg, command, width=None): # helper to create a styled button
        return Button(
    ...
    def _build_action_buttons(self): #Claude helped with this subroutine to create all the buttons for the different games and the shop
    ...
    def _build_nav_panel(self): # builds the directional navigation buttons in the nav panel at the bottom of the window
        ...
        # helper to create a navigation button with consistent styling and command (Claude helped me improve my original code by creating this helper function and adding the arrow symbols to the button text)
        ...
        #makes the actual buttons and places them in a grid to form a directional pad layout (up, down, left, right)
    ...
    def _refresh_use_buttons(self): #vibe coded the buttons with Claude
        ...
        items = Casino_of_Sin_Final.inventory
        if not items:
            return
    ...
    def hit_or_stand(self, choice):
        ...
        Casino_of_Sin_Final.choice = choice
        msg = Casino_of_Sin_Final.playBlackjack()
        self.result_label.config(text=msg if msg else "")
        self.money_label.config(text=f"Money:  ${Casino_of_Sin_Final.money}")
        if not Casino_of_Sin_Final.blackjackState["active"]:
    ...
    def red_or_black(self, answer):
        _play(SND_BET)
        Casino_of_Sin_Final.answer = answer
        msg = Casino_of_Sin_Final.playRoulette()
        self.result_label.config(text=msg if msg else "")
        self.money_label.config(text=f"Money:  ${Casino_of_Sin_Final.money}")
        self._hide_all_actions()
        self.play_button.grid(row=0, column=0, columnspan=2, sticky="ew", pady=4)
    ...
    def buy_item(self, item_name):
        _play(SND_BUY)
        msg = Casino_of_Sin_Final.buyItem(item_name)
        self.result_label.config(text=msg)
        self.money_label.config(text=f"Money:  ${Casino_of_Sin_Final.money}")
        self.inventory_label.config(text=f"Inventory:  {Casino_of_Sin_Final.inventory}")
        self._refresh_use_buttons()

    def use_item(self, item_name):
        _play(SND_USE)
        msg = Casino_of_Sin_Final.useItem(item_name)
        self.result_label.config(text=msg)
        self.inventory_label.config(text=f"Inventory:  {Casino_of_Sin_Final.inventory}")
        self._refresh_use_buttons()

    def play_game(self): #call this to play the game in the current room
        _play(SND_PLAY)
        if self.currentRoom.name == "Blackjack Room":
            ...
            msg = Casino_of_Sin_Final.startBlackjack()
            self.result_label.config(text=msg if msg else "")
            self.money_label.config(text=f"Money:  ${Casino_of_Sin_Final.money}")
    ...
    #helped by Claude to create this helper function that hides all the action buttons (better than uses grid.remove() on each button individually in multiple places in the code, which was what I was doing before and it was very messy)
    ...
    def _trigger_end_state(self, state):
        ...
        if state == "death":
            ...
            result_text = f"Final balance: ${Casino_of_Sin_Final.money}"
        else:
            ...
            result_text = f"Final balance: ${Casino_of_Sin_Final.money} — Humanity saved!"
    ...
    def _check_end_state(self):
        """Trigger the appropriate end screen if the game is over."""
        if Casino_of_Sin_Final.money <= 0:
            self._trigger_end_state("death")
            return True
        elif Casino_of_Sin_Final.money >= 10000:
            self._trigger_end_state("victory")
            return True
        return False

    #updates the room after moving to a new room or after playing a game
    def update_room_display(self):
        ...
        if self.currentRoom.game is not None: #if there is no game in the room, display no button
            self.play_button.grid(row=0, column=0, columnspan=2, sticky="ew", pady=4)
        elif self.currentRoom.name == "Bar":
            self.buy_button.grid(row=0, column=0, columnspan=2, sticky="ew", pady=4)

        # Check for death / victory after all display updates
        self._check_end_state()
