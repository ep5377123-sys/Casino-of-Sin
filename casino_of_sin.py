...
# Blackjack state — used by the GUI to track a game across multiple button presses
blackjackState = {
    "active": False,
    "deck": [],
    "player_card": [],
...
def startBlackjack(): #calls the black jack game. got this code from https://www.geeksforgeeks.org/python/blackjack-console-game-using-python/
    ...
    # Save state so Hit/Stand buttons can continue the hand
...
def playBlackjack(): #called when the player clicks Hit or Stand. Uses the global `choice` variable.
